...
function displayPaco() {
    include("inc/config.php");

    $sql = "SELECT * FROM paco";
...
function displayTutubanPaco() {
    include("inc/config.php");

    $sql = "SELECT * FROM tutuban_paco";
    $result = $conn->query($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_POST['direction'] == "Northbound") {
        ...
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM paco WHERE time = ?");
    } elseif ($_POST['direction'] == "Southbound") {
        // Check if the time already exists in the database
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM tutuban_paco WHERE time = ?");
    } else {
    ...
    if ($count > 0) {
        ...
        echo "<script>alert('Record with the same time already exists'); window.location = 'schedule_paco.php';</script>";
    } else {
        // Proceed with database insertion
        if ($_POST['direction'] == "Northbound") {
            $stmt = $conn->prepare("INSERT INTO paco (time, status, direction) VALUES (?, ?, ?)");
        } elseif ($_POST['direction'] == "Southbound") {
            $stmt = $conn->prepare("INSERT INTO tutuban_paco (time, status, direction) VALUES (?, ?, ?)");
        } else {
            // Handle invalid direction
            echo '<script>alert("Invalid direction selected");</script>';
            exit(); // Stop further execution
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Record added successfully'); window.location = 'schedule_paco.php';</script>";
...
<div class="modal fade" id="confirmResetTrainModal" tabindex="-1" aria-labelledby="deleteModalTutubanLabel" aria-hidden="true">
   ...
   <div class="content ">
      ...
      <div class="row">
            ...
            <li class="nav-item">
               <a class="nav-link active" data-bs-toggle="tab" href="#PacotoTutuban" style="color:#FFC30B;font-weight:900;">Paco to Tutuban</a>
            </li>
            <li class="nav-item">
               <a class="nav-link" data-bs-toggle="tab" href="#TutubantoPaco" style="color: #FFC30B;font-weight:900;">Tutuban to Paco</a>
            </li>
         </ul>
         ...
         <div class="tab-content">
            <div id="PacotoTutuban" class="tab-pane fade show active">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainPaco" name="updateTrainPaco" required onchange="updateTrainPaco()">
            ...
            <table id="paco" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
         <div class="modal-dialog">
            <div class="modal-content">
               ...
               <div class="modal-body">
                  <form id="editForm" action="edit_schedule_paco.php" method="post">
            ...
            <div id="TutubantoPaco" class="tab-pane fade">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainTutubanPaco" name="updateTrainTutubanPaco" required onchange="updateTrainTutubanPaco()">
            <option value="" disabled selected>Select Train</option>
                <?php
                foreach ($trains as $train) {
                    echo '<option value="' . $train['train_value'] . '" data-station-id="' . $train['train_no'] . '">' . $train['train_no'] . '</option>';
               ...
               <table id="tutuban_paco" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
            <th style="background-color:#174793;color:#FFF;">Select</th>
            <th style="background-color:#174793;color:#FFF;">No.</th>
...
<div class="modal fade" id="editModalTutuban" tabindex="-1" aria-labelledby="editModalTutubanLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editFormTutuban" action="edit_schedule_tutuban_paco.php" method="post">
...
function downloadTables() {
    ...
    const table1 = $('#paco').DataTable().rows().data().toArray();
    const table2 = $('#tutuban_paco').DataTable().rows().data().toArray();
    ...
    filteredData1.unshift(['Paco to Tutuban', `Date: ${formattedDate}`]);
    ...
    filteredData2.unshift(['Tutuban to Paco', `Date: ${formattedDate}`]);
    ...
    link.setAttribute("download", `schedule_paco_tutuban_${formattedDate}.csv`);
...
<script>
    function updateTrainPaco() {
        var selectedTrain = document.getElementById("updateTrainPaco").value;
...
<script>
    function updateTrainTutubanPaco() {
        var selectedTrain = document.getElementById("updateTrainTutubanPaco").value;
        ...
        if (selectedTrain !== "" && selectedSchedules.length > 0) {
            $.ajax({
                type: 'POST',
                url: 'update_train_tutuban_paco.php',
                data: { train: selectedTrain, selectedSchedules: selectedSchedules },
                success: function (response) {
                    location.reload();
                },
