...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['train']) && isset($_POST['selectedSchedules'])) {
    ...
    $stmtTutuban = $conn->prepare("UPDATE tutuban SET train = ? WHERE schedule_id = ?");
    $stmtTutuban->bind_param("si", $selectedTrain, $scheduleId);
    ...
    $stmtBluementritt = $conn->prepare("UPDATE tutuban_bluementritt SET train = ? WHERE schedule_id = ?");
