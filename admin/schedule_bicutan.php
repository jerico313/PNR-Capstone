...
function displayBicutan() {
    include("inc/config.php");

    $sql = "SELECT * FROM bicutan";
...
function displayTutubanBicutan() {
    include("inc/config.php");

    $sql = "SELECT * FROM tutuban_bicutan";
    $result = $conn->query($sql);
...
if ($_POST['direction'] == "Northbound") {
    ...
    $checkStmt = $conn->prepare("SELECT COUNT(*) FROM bicutan WHERE time = ?");
} elseif ($_POST['direction'] == "Southbound") {
    // Check if the time already exists in the database
    $checkStmt = $conn->prepare("SELECT COUNT(*) FROM tutuban_bicutan WHERE time = ?");
} else {
...
if ($count > 0) {
    ...
    echo "<script>alert('Record with the same time already exists'); window.location = 'schedule_bicutan.php';</script>";
} else {
    // Proceed with database insertion
    if ($_POST['direction'] == "Northbound") {
        $stmt = $conn->prepare("INSERT INTO bicutan (time, status, direction) VALUES (?, ?, ?)");
    } elseif ($_POST['direction'] == "Southbound") {
        $stmt = $conn->prepare("INSERT INTO tutuban_bicutan (time, status, direction) VALUES (?, ?, ?)");
    } else {
        // Handle invalid direction
        echo '<script>alert("Invalid direction selected");</script>';
        exit(); // Stop further execution
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>alert('Record added successfully'); window.location = 'schedule_bicutan.php';</script>";
        if ($_POST['direction'] == "Southbound") {
            echo "<script>displayTutubanBicutan();</script>";
        } elseif ($_POST['direction'] == "Northbound") {
            echo "<script>displayBicutan();</script>"; 
        }
    } else {
...
<div class="modal fade" id="confirmResetTrainModal" tabindex="-1" aria-labelledby="deleteModalTutubanLabel" aria-hidden="true">
   ...
   <div class="content ">
      ...
      <div class="row">
            ...
            <li class="nav-item">
               <a class="nav-link active" data-bs-toggle="tab" href="#BicutantoTutuban" style="color:#FFC30B;font-weight:900;">Bicutan to Tutuban</a>
            </li>
            <li class="nav-item">
               <a class="nav-link" data-bs-toggle="tab" href="#TutubantoBicutan" style="color: #FFC30B;font-weight:900;">Tutuban to Bicutan</a>
            </li>
         </ul>
         ...
         <div class="tab-content">
            <div id="BicutantoTutuban" class="tab-pane fade show active">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainBicutan" name="updateTrainBicutan" required onchange="updateTrainBicutan()">
            ...
            <table id="bicutan" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
         <div class="modal-dialog">
            <div class="modal-content">
               ...
               <div class="modal-body">
                  <form id="editForm" action="edit_schedule_bicutan.php" method="post">
            ...
            <div id="TutubantoBicutan" class="tab-pane fade">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainTutubanBicutan" name="updateTrainTutubanBicutan" required onchange="updateTrainTutubanBicutan()">
                <option value="" disabled selected>Select Train</option>
                    <?php
                    foreach ($trains as $train) {
                        echo '<option value="' . $train['train_value'] . '" data-station-id="' . $train['train_no'] . '">' . $train['train_no'] . '</option>';
               ...
               <table id="tutuban_bicutan" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
            <th style="background-color:#174793;color:#FFF;">Select</th>
            <th style="background-color:#174793;color:#FFF;">No.</th>
...
<div class="modal fade" id="editModalTutuban" tabindex="-1" aria-labelledby="editModalTutubanLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editFormTutuban" action="edit_schedule_tutuban_bicutan.php" method="post">
...
function downloadTables() {
    ...
    const table1 = $('#bicutan').DataTable().rows().data().toArray();
    const table2 = $('#tutuban_bicutan').DataTable().rows().data().toArray();
    ...
    filteredData1.unshift(['Bicutan to Tutuban', `Date: ${formattedDate}`]);
    ...
    filteredData2.unshift(['Tutuban to Bicutan', `Date: ${formattedDate}`]);
    ...
    link.setAttribute("download", `schedule_bicutan_tutuban_${formattedDate}.csv`);
...
function editTutuban(scheduleId) {
    ...
    function confirmDeleteTutuban() {
        ...
        // Add logic for deleting a schedule
        $.ajax({
            url: 'delete_tutuban_bicutan.php',
            method: 'POST',
            data: { scheduleId: scheduleId },
            success: function(response) {
                // Log the response from the server
...
<script>
    function updateTrainBicutan() {
        var selectedTrain = document.getElementById("updateTrainBicutan").value;
...
<script>
    function updateTrainTutubanBicutan() {
        var selectedTrain = document.getElementById("updateTrainTutubanBicutan").value;
        ...
        if (selectedTrain !== "" && selectedSchedules.length > 0) {
            $.ajax({
                type: 'POST',
                url: 'update_train_tutuban_Bicutan.php',
                data: { train: selectedTrain, selectedSchedules: selectedSchedules },
                success: function (response) {
                    location.reload();
                },
