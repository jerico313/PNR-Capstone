...
function displayBluementritt() {
    include("inc/config.php");

    $sql = "SELECT * FROM bluementritt";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td><center><input type='checkbox' class='schedule-checkbox' value='" . $row['schedule_id'] . "'></center></td>";
            echo "<td>" . $count . "</td>";
            echo "<td>" . $row['time'] . "</td>";
...
function displayTutubanBluementritt() {
    include("inc/config.php");

    $sql = "SELECT * FROM tutuban_bluementritt";
    $result = $conn->query($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_POST['direction'] == "Northbound") {
        ...
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM bluementritt WHERE time = ?");
    } elseif ($_POST['direction'] == "Southbound") {
        // Check if the time already exists in the database
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM tutuban_bluementritt WHERE time = ?");
    } else {
    ...
    if ($count > 0) {
        ...
        echo "<script>alert('Record with the same time already exists'); window.location = 'schedule_bluementritt.php';</script>";
    } else {
        // Proceed with database insertion
        if ($_POST['direction'] == "Northbound") {
            $stmt = $conn->prepare("INSERT INTO bluementritt (time, status, direction) VALUES (?, ?, ?)");
        } elseif ($_POST['direction'] == "Southbound") {
            $stmt = $conn->prepare("INSERT INTO tutuban_bluementritt (time, status, direction) VALUES (?, ?, ?)");
        } else {
            // Handle invalid direction
            echo '<script>alert("Invalid direction selected");</script>';
            exit(); // Stop further execution
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Record added successfully'); window.location = 'schedule_bluementritt.php';</script>";
            if ($_POST['direction'] == "Southbound") {
                echo "<script>displayTutubanBluementritt();</script>"; // Call the function to display data for Alabang
            } elseif ($_POST['direction'] == "Northbound") {
                echo "<script>displayBluementritt();</script>"; // Call the function to display data for Tutuban
            }
        } else {
    ...
    function getTrains() {
        ...
        $sql = "SELECT * FROM trains"; // Assuming the table name is "trains"
        $result = $conn->query($sql);
...
<div class="reset" style="padding-top: 120px; padding-bottom: 0px; text-align: right;padding-right:30px">
    ...
    <button type="button" class="btn btn-danger" id="resetTrainButton" style="display: inline-block;  margin-right: 10px;">Reset Train No.</button>
...
<div class="modal fade" id="confirmResetTrainModal" tabindex="-1" aria-labelledby="deleteModalTutubanLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="backdrop-filter: blur(16px) saturate(180%);-webkit-backdrop-filter: blur(16px) saturate(180%);background-color: rgba(255, 255, 255, 0.40);border-radius: 12px;border: 1px solid rgba(209, 213, 219, 0.3);">
            <div class="modal-body">
                <input type="hidden" id="deleteScheduleId" name="deleteScheduleId" value="">
                <center>
                    <p style="padding:10px">Are you sure you want to reset the Train?</p>
                </center>
            </div>
            <div class="modal-footer  text-center">
                <button type="button" class="btn" data-bs-dismiss="modal" style="background-color:transparent;margin: 0 auto;">Cancel</button>
                <button type="button" class="btn" id="confirmResetTrainBtn" style="background-color:transparent;margin: 0 auto;color: red;">Reset</button>
            </div>
        </div>
    </div>
</div>
   <div class="content ">
      ...
      <div class="row">
            ...
            <li class="nav-item">
               <a class="nav-link active" data-bs-toggle="tab" href="#BluementritttoTutuban" style="color:#FFC30B;font-weight:900;">Bluemenritt to Tutuban</a>
            </li>
            <li class="nav-item">
         ...
         <div class="tab-content">
            <div id="BluementritttoTutuban" class="tab-pane fade show active">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainBluementritt" name="updateTrainBluementritt" required onchange="updateTrainBluementritt()">
            <option value="" disabled selected>Select Train</option>
                <?php
                foreach ($trains as $train) {
                    echo '<option value="' . $train['train_value'] . '" data-station-id="' . $train['train_no'] . '">' . $train['train_no'] . '</option>';
                }
                ?>
            </select>
            </div>
            <table id="bluementritt" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
            <th style="background-color:#174793;color:#FFF;">Select</th>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
         <div class="modal-dialog">
            <div class="modal-content">
               ...
               <div class="modal-body">
                  <form id="editForm" action="edit_schedule_bluementritt.php" method="post">
            ...
            <div id="TutubantoBluementritt" class="tab-pane fade">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainTutubanBluementritt" name="updateTrainTutubanBluementritt" required onchange="updateTrainTutubanBluementritt()">
            <option value="" disabled selected>Select Train</option>
                <?php
                foreach ($trains as $train) {
                    echo '<option value="' . $train['train_value'] . '" data-station-id="' . $train['train_no'] . '">' . $train['train_no'] . '</option>';
               ...
               <table id="tutuban_bluementritt" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
            <th style="background-color:#174793;color:#FFF;">Select</th>
            <th style="background-color:#174793;color:#FFF;">No.</th>
        ...
        <?php displayTutubanBluementritt();?>
...
<div class="modal fade" id="editModalTutuban" tabindex="-1" aria-labelledby="editModalTutubanLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editFormTutuban" action="edit_schedule_tutuban_bluementritt.php" method="post">
...
<script src='https://cdn.datatables.net/1.13.5/js/dataTables.bootstrap5.min.js'></script>
      ...
      <script>
         ...
         $(document).ready(function() {
             $('#tutuban_bluementritt').DataTable({
                 //disable sorting on last column
                 "columnDefs": [
                     { "orderable": false, "targets": 4 }
                 ],
...
function downloadTables() {
    ...
    const table1 = $('#bluementritt').DataTable().rows().data().toArray();
    const table2 = $('#tutuban_bluementritt').DataTable().rows().data().toArray();
    ...
    filteredData1.unshift(['Bluementritt to Tutuban', `Date: ${formattedDate}`]);
    ...
    filteredData2.unshift(['Tutuban to Bluementritt', `Date: ${formattedDate}`]);
    ...
    link.setAttribute("download", `schedule_bluementritt_tutuban_${formattedDate}.csv`);
...
function editTutuban(scheduleId) {
    ...
    function confirmDeleteTutuban() {
        ...
        // Add logic for deleting a schedule
        $.ajax({
            url: 'delete_tutuban_bluementritt.php',
            method: 'POST',
            data: { scheduleId: scheduleId },
            success: function(response) {
                // Log the response from the server
...
<script>
    $(document).ready(function () {
        ...
        $('#confirmResetBtn').click(function () {
            ...
            $.ajax({
                url: 'reset_status_bluementritt.php',
                method: 'POST',
                success: function (response) {
                    console.log(response);
...
<script>
    $(document).ready(function () {
        ...
        $('#resetTrainButton').click(function () {
            // Open the confirmation modal
            $('#confirmResetTrainModal').modal('show');
        });

        // Handle reset confirmation
        $('#confirmResetTrainBtn').click(function () {
            // Make an AJAX request to reset_status.php
            $.ajax({
                url: 'reset_train.php',
                method: 'POST',
            ...
            // Close the confirmation modal
            $('#confirmResetTrainModal').modal('hide');
        });
...
<script>
    function updateTrainBluementritt() {
        var selectedTrain = document.getElementById("updateTrainBluementritt").value;
        ...
        if (selectedTrain !== "" && selectedSchedules.length > 0) {
            $.ajax({
                type: 'POST',
                url: 'update_train_bluementritt.php',
                data: { train: selectedTrain, selectedSchedules: selectedSchedules },
                success: function (response) {
                    location.reload();
                },
...
<script>
    function updateTrainTutubanBluementritt() {
        var selectedTrain = document.getElementById("updateTrainTutubanBluementritt").value;
        ...
        if (selectedTrain !== "" && selectedSchedules.length > 0) {
            $.ajax({
                type: 'POST',
                url: 'update_train_tutuban_bluementritt.php',
                data: { train: selectedTrain, selectedSchedules: selectedSchedules },
                success: function (response) {
                    location.reload();
                },
