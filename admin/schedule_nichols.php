...
function displayNichols() {
    include("inc/config.php");

    $sql = "SELECT * FROM nichols";
...
function displayTutubanNichols() {
    include("inc/config.php");

    $sql = "SELECT * FROM tutuban_nichols";
    $result = $conn->query($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_POST['direction'] == "Northbound") {
        ...
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM nichols WHERE time = ?");
    } elseif ($_POST['direction'] == "Southbound") {
        // Check if the time already exists in the database
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM tutuban_nichols WHERE time = ?");
    } else {
    ...
    if ($count > 0) {
        ...
        echo "<script>alert('Record with the same time already exists'); window.location = 'schedule_nichols.php';</script>";
    } else {
        // Proceed with database insertion
        if ($_POST['direction'] == "Northbound") {
            $stmt = $conn->prepare("INSERT INTO nichols (time, status, direction) VALUES (?, ?, ?)");
        } elseif ($_POST['direction'] == "Southbound") {
            $stmt = $conn->prepare("INSERT INTO tutuban_nichols (time, status, direction) VALUES (?, ?, ?)");
        } else {
            // Handle invalid direction
            echo '<script>alert("Invalid direction selected");</script>';
            exit(); // Stop further execution
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Record added successfully'); window.location = 'schedule_nichols.php';</script>";
            if ($_POST['direction'] == "Southbound") {
                echo "<script>displayTutuban();</script>"; // Call the function to display data for Alabang
            } elseif ($_POST['direction'] == "Northbound") {
                echo "<script>displaySchedule();</script>"; // Call the function to display data for Tutuban
            }
        } else {
...
<div class="modal fade" id="confirmResetTrainModal" tabindex="-1" aria-labelledby="deleteModalTutubanLabel" aria-hidden="true">
   ...
   <div class="content ">
      ...
      <div class="row">
            ...
            <li class="nav-item">
               <a class="nav-link active" data-bs-toggle="tab" href="#NicholstoTutuban" style="color:#FFC30B;font-weight:900;">Nichols to Tutuban</a>
            </li>
            <li class="nav-item">
               <a class="nav-link" data-bs-toggle="tab" href="#TutubantoNichols" style="color: #FFC30B;font-weight:900;">Tutuban to Nichols</a>
            </li>
         </ul>
         ...
         <div class="tab-content">
            <div id="NicholstoTutuban" class="tab-pane fade show active">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainNichols" name="updateTrainNichols" required onchange="updateTrainNichols()">
            ...
            <table id="nichols" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
         <div class="modal-dialog">
            <div class="modal-content">
               ...
               <div class="modal-body">
                  <form id="editForm" action="edit_schedule_nichols.php" method="post">
            ...
            <div id="TutubantoNichols" class="tab-pane fade">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainTutubanNichols" name="updateTrainTutubanNichols" required onchange="updateTrainTutubanNichols()">
            <option value="" disabled selected>Select Train</option>
                <?php
                foreach ($trains as $train) {
                    echo '<option value="' . $train['train_value'] . '" data-station-id="' . $train['train_no'] . '">' . $train['train_no'] . '</option>';
               ...
               <table id="tutuban_nichols" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
            <th style="background-color:#174793;color:#FFF;">Select</th>
            <th style="background-color:#174793;color:#FFF;">No.</th>
...
<div class="modal fade" id="editModalTutuban" tabindex="-1" aria-labelledby="editModalTutubanLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editFormTutuban" action="edit_schedule_tutuban_nichols.php" method="post">
...
function downloadTables() {
    ...
    const table1 = $('#nichols').DataTable().rows().data().toArray();
    const table2 = $('#tutuban_nichols').DataTable().rows().data().toArray();
    ...
    filteredData1.unshift(['Nichols to Tutuban', `Date: ${formattedDate}`]);
    ...
    filteredData2.unshift(['Tutuban to Nichols', `Date: ${formattedDate}`]);
    ...
    link.setAttribute("download", `schedule_nichols_tutuban_${formattedDate}.csv`);
...
function editTutuban(scheduleId) {
    ...
    function confirmDeleteTutuban() {
        ...
        // Add logic for deleting a schedule
        $.ajax({
            url: 'delete_tutuban_nichols.php',
            method: 'POST',
            data: { scheduleId: scheduleId },
            success: function(response) {
                // Log the response from the server
...
<script>
    function updateTrainNichols() {
        var selectedTrain = document.getElementById("updateTrainNichols").value;
...
<script>
    function updateTrainTutubanNichols() {
        var selectedTrain = document.getElementById("updateTrainTutubanNichols").value;
        ...
        if (selectedTrain !== "" && selectedSchedules.length > 0) {
            $.ajax({
                type: 'POST',
                url: 'update_train_tutuban_nichols.php',
                data: { train: selectedTrain, selectedSchedules: selectedSchedules },
                success: function (response) {
                    location.reload();
                },
