...
function displaySucat() {
    include("inc/config.php");

    $sql = "SELECT * FROM sucat";
...
function displayTutubanSucat() {
    include("inc/config.php");

    $sql = "SELECT * FROM tutuban_sucat";
    $result = $conn->query($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_POST['direction'] == "Northbound") {
        ...
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM sucat WHERE time = ?");
    } elseif ($_POST['direction'] == "Southbound") {
        // Check if the time already exists in the database
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM tutuban_sucat WHERE time = ?");
    } else {
    ...
    if ($count > 0) {
        ...
        echo "<script>alert('Record with the same time already exists'); window.location = 'schedule_sucat.php';</script>";
    } else {
        // Proceed with database insertion
        if ($_POST['direction'] == "Northbound") {
            $stmt = $conn->prepare("INSERT INTO sucat (time, status, direction) VALUES (?, ?, ?)");
        } elseif ($_POST['direction'] == "Southbound") {
            $stmt = $conn->prepare("INSERT INTO tutuban_sucat (time, status, direction) VALUES (?, ?, ?)");
        } else {
            // Handle invalid direction
            echo '<script>alert("Invalid direction selected");</script>';
            exit(); // Stop further execution
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Record added successfully'); window.location = 'schedule_sucat.php';</script>";
            if ($_POST['direction'] == "Southbound") {
                echo "<script>displayTutubanSucat();</script>"; // Call the function to display data for Alabang
            } elseif ($_POST['direction'] == "Northbound") {
                echo "<script>displaySucat();</script>"; // Call the function to display data for Tutuban
            }
        } else {
...
<div class="modal fade" id="confirmResetTrainModal" tabindex="-1" aria-labelledby="deleteModalTutubanLabel" aria-hidden="true">
   ...
   <div class="content ">
      ...
      <div class="row">
            ...
            <li class="nav-item">
               <a class="nav-link active" data-bs-toggle="tab" href="#SucattoTutuban" style="color:#FFC30B;font-weight:900;">Sucat to Tutuban</a>
            </li>
            <li class="nav-item">
               <a class="nav-link" data-bs-toggle="tab" href="#TutubantoSucat" style="color: #FFC30B;font-weight:900;">Tutuban to Sucat</a>
            </li>
         </ul>
         ...
         <div class="tab-content">
            <div id="SucattoTutuban" class="tab-pane fade show active">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainSucat" name="updateTrainSucat" required onchange="updateTrainSucat()">
    ...
    ?>
            ...
            <table id="sucat" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
         <div class="modal-dialog">
            <div class="modal-content">
               ...
               <div class="modal-body">
                  <form id="editForm" action="edit_schedule_sucat.php" method="post">
            ...
            <div id="TutubantoSucat" class="tab-pane fade">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainTutubanSucat" name="updateTrainTutubanSucat" required onchange="updateTrainTutubanSucat()">
    <option value="" disabled selected>Select Train</option>
    <?php
    foreach ($trains as $train) {
        echo '<option value="' . $train['train_value'] . '" data-station-id="' . $train['train_no'] . '">' . $train['train_no'] . '</option>';
    ...
    ?>
               ...
               <table id="tutuban_sucat" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
            <th style="background-color:#174793;color:#FFF;">Select</th>
            <th style="background-color:#174793;color:#FFF;">No.</th>
...
<div class="modal fade" id="editModalTutuban" tabindex="-1" aria-labelledby="editModalTutubanLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editFormTutuban" action="edit_schedule_tutuban_sucat.php" method="post">
...
function downloadTables() {
    ...
    const table1 = $('#sucat').DataTable().rows().data().toArray();
    const table2 = $('#tutuban_sucat').DataTable().rows().data().toArray();
    ...
    filteredData1.unshift(['Sucat to Tutuban', `Date: ${formattedDate}`]);
    ...
    filteredData2.unshift(['Tutuban to Sucat', `Date: ${formattedDate}`]);
    ...
    link.setAttribute("download", `schedule_sucat_tutuban_${formattedDate}.csv`);
...
<script>
    function updateTrain() {
        var selectedTrain = document.getElementById("updateTrain").value;
...
<script>
    function updateTrainSucat() {
        var selectedTrain = document.getElementById("updateTrainSucat").value;
...
<script>
    function updateTrainTutubanSucat() {
        var selectedTrain = document.getElementById("updateTrainTutubanSucat").value;
        ...
        if (selectedTrain !== "" && selectedSchedules.length > 0) {
            $.ajax({
                type: 'POST',
                url: 'update_train_tutuban_sucat.php',
                data: { train: selectedTrain, selectedSchedules: selectedSchedules },
                success: function (response) {
                    location.reload();
                },
