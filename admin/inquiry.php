...
function displayInquiryData() {
   ...
   $sql = "SELECT * FROM inquiry"; // Change table name from inquiry to inbox
   $result = $conn->query($sql);

   if ($result->num_rows > 0) {
       $count = 1;
       while ($row = $result->fetch_assoc()) {
           echo "<tr data-id='" . $row['inquiry_id'] . "' class='message-row'>";
           ...
           echo "<td>";
           echo "<button class='btn btn-success btn-sm' onclick='ReplyMessage(" . $row['inquiry_id'] . ")'>Reply</button> ";
           echo "<button class='btn btn-danger btn-sm' onclick='deleteMessage(" . $row['inquiry_id'] . ")'>Delete</button>";
           echo "</td>";
           echo "</tr>";
           $count++;
       }
            ...
            <div class="row">
               <div class="col-md-12 pt-5">
                  <div class="custom-box yellow-box">
                     <p style="color:#FFF;padding: 15px 10px 0 9px;"><strong>Inquiries Inbox</strong></p>
                  </div>
...
$(document).ready(function() {
   ...
   $('#example tbody').on('click', 'td:nth-child(1), td:nth-child(2), td:nth-child(3), td:nth-child(4)', function () {
      ...
      $.ajax({
         url: 'get_inquiry_details.php', // Replace with the actual PHP file to fetch details
         method: 'POST',
...
<script>
   function ReplyMessage(inquiryId) {
    // Fetch the email using AJAX
    $.ajax({
        url: 'get_inquiry_email.php', // Replace with the actual PHP file to fetch email
        method: 'POST',
        data: { inquiryId: inquiryId },
        success: function(response) {
      ...
      <script>
         function deleteMessage(inquiryId) {
             // Set the train ID to be deleted
             $('#deleteModal').data('inquiryId', inquiryId);
         ...
         function confirmDelete() {
             ...
             var inquiryId = $('#deleteModal').data('inquiryId');
