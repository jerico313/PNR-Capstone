...
function displaySantaMesa() {
    include("inc/config.php");

    $sql = "SELECT * FROM santamesa";
...
function displayTutubanSantaMesa() {
    include("inc/config.php");

    $sql = "SELECT * FROM tutuban_santamesa";
    $result = $conn->query($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_POST['direction'] == "Northbound") {
        ...
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM santamesa WHERE time = ?");
    } elseif ($_POST['direction'] == "Southbound") {
        // Check if the time already exists in the database
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM tutuban_santamesa WHERE time = ?");
    } else {
    ...
    if ($count > 0) {
        ...
        echo "<script>alert('Record with the same time already exists'); window.location = 'schedule_santamesa.php';</script>";
    } else {
        // Proceed with database insertion
        if ($_POST['direction'] == "Northbound") {
            $stmt = $conn->prepare("INSERT INTO santamesa (time, status, direction) VALUES (?, ?, ?)");
        } elseif ($_POST['direction'] == "Southbound") {
            $stmt = $conn->prepare("INSERT INTO tutuban_santamesa (time, status, direction) VALUES (?, ?, ?)");
        } else {
            // Handle invalid direction
            echo '<script>alert("Invalid direction selected");</script>';
            exit(); // Stop further execution
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Record added successfully'); window.location = 'schedule_santamesa.php';</script>";
            if ($_POST['direction'] == "Southbound") {
                echo "<script>displayTutubanSantaMesa();</script>"; // Call the function to display data for Alabang
            } elseif ($_POST['direction'] == "Northbound") {
                echo "<script>displaySantaMesa();</script>"; // Call the function to display data for Tutuban
            }
        } else {
...
<div class="modal fade" id="confirmResetTrainModal" tabindex="-1" aria-labelledby="deleteModalTutubanLabel" aria-hidden="true">
   ...
   <div class="content ">
      ...
      <div class="row">
            ...
            <li class="nav-item">
               <a class="nav-link active" data-bs-toggle="tab" href="#SantaMesatoTutuban" style="color:#FFC30B;font-weight:900;">Santa Mesa to Tutuban</a>
            </li>
            <li class="nav-item">
               <a class="nav-link" data-bs-toggle="tab" href="#TutubantoSantaMesa" style="color: #FFC30B;font-weight:900;">Tutuban to Santa Mesa</a>
            </li>
         </ul>
         ...
         <div class="tab-content">
            <div id="SantaMesatoTutuban" class="tab-pane fade show active">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainSantaMesa" name="updateTrainSantaMesa" required onchange="updateTrainSantaMesa()">
            ...
            <table id="santamesa" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
         <div class="modal-dialog">
            <div class="modal-content">
               ...
               <div class="modal-body">
                  <form id="editForm" action="edit_schedule_santamesa.php" method="post">
            ...
            <div id="TutubantoSantaMesa" class="tab-pane fade">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainTutubanSantaMesa" name="updateTrainTutubanSantaMesa" required onchange="updateTrainTutubanSantaMesa()">
            <option value="" disabled selected>Select Train</option>
                <?php
                foreach ($trains as $train) {
                    echo '<option value="' . $train['train_value'] . '" data-station-id="' . $train['train_no'] . '">' . $train['train_no'] . '</option>';
               ...
               <table id="tutuban_santamesa" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
            <th style="background-color:#174793;color:#FFF;">Select</th>
            <th style="background-color:#174793;color:#FFF;">No.</th>
...
<div class="modal fade" id="editModalTutuban" tabindex="-1" aria-labelledby="editModalTutubanLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editFormTutuban" action="edit_schedule_tutuban_santamesa.php" method="post">
...
function downloadTables() {
    ...
    const table1 = $('#santamesa').DataTable().rows().data().toArray();
    const table2 = $('#tutuban_santamesa').DataTable().rows().data().toArray();
    ...
    filteredData1.unshift(['Santa Mesa to Tutuban', `Date: ${formattedDate}`]);
    ...
    filteredData2.unshift(['Tutuban to Santa Mesa', `Date: ${formattedDate}`]);
    ...
    link.setAttribute("download", `schedule_santa_mesa_tutuban_${formattedDate}.csv`);
...
function editTutuban(scheduleId) {
    ...
    function confirmDeleteTutuban() {
        ...
        // Add logic for deleting a schedule
        $.ajax({
            url: 'delete_tutuban_santamesa.php',
            method: 'POST',
            data: { scheduleId: scheduleId },
            success: function(response) {
                // Log the response from the server
...
<script>
    $(document).ready(function () {
        ...
        $('#confirmResetBtn').click(function () {
            ...
            $.ajax({
                url: 'reset_status_santamesa.php',
                method: 'POST',
                success: function (response) {
                    console.log(response);
...
<script>
    function updateTrainSantaMesa() {
        var selectedTrain = document.getElementById("updateTrainSantaMesa").value;
        ...
        if (selectedTrain !== "" && selectedSchedules.length > 0) {
            $.ajax({
                type: 'POST',
                url: 'update_train_santamesa.php',
...
<script>
    function updateTrainTutubanSantaMesa() {
        var selectedTrain = document.getElementById("updateTrainTutubanSantaMesa").value;
        ...
        if (selectedTrain !== "" && selectedSchedules.length > 0) {
            $.ajax({
                type: 'POST',
                url: 'update_train_tutuban_santamesa.php',
                data: { train: selectedTrain, selectedSchedules: selectedSchedules },
                success: function (response) {
                    location.reload();
                },
