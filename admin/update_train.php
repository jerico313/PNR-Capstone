...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['train']) && isset($_POST['selectedSchedules'])) {
    ...
    // Use a prepared statement to update each selected schedule for table "alabang"
    $stmtAlabang = $conn->prepare("UPDATE alabang SET train = ? WHERE schedule_id = ?");
    $stmtAlabang->bind_param("si", $selectedTrain, $scheduleId);

    // Use a prepared statement to update each selected schedule for "tutuban" table
    $stmtSucat = $conn->prepare("UPDATE sucat SET train = ? WHERE schedule_id = ?");
