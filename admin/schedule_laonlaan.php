...
function displayLaonLaan() {
    include("inc/config.php");

    $sql = "SELECT * FROM laonlaan";
...
function displayTutubanLaonLaan() {
    include("inc/config.php");

    $sql = "SELECT * FROM tutuban_laonlaan";
    $result = $conn->query($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_POST['direction'] == "Northbound") {
        ...
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM laonlaan WHERE time = ?");
    } elseif ($_POST['direction'] == "Southbound") {
        // Check if the time already exists in the database
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM tutuban_laonlaan WHERE time = ?");
    } else {
    ...
    if ($count > 0) {
        ...
        echo "<script>alert('Record with the same time already exists'); window.location = 'schedule_laonlaan.php';</script>";
    } else {
        // Proceed with database insertion
        if ($_POST['direction'] == "Northbound") {
            $stmt = $conn->prepare("INSERT INTO laonlaan (time, status, direction) VALUES (?, ?, ?)");
        } elseif ($_POST['direction'] == "Southbound") {
            $stmt = $conn->prepare("INSERT INTO tutuban_laonlaan (time, status, direction) VALUES (?, ?, ?)");
        } else {
            // Handle invalid direction
            echo '<script>alert("Invalid direction selected");</script>';
            exit(); // Stop further execution
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Record added successfully'); window.location = 'schedule_laonlaan.php';</script>";
            if ($_POST['direction'] == "Southbound") {
                echo "<script>displayTutubanLaonLaan();</script>"; // Call the function to display data for Alabang
            } elseif ($_POST['direction'] == "Northbound") {
                echo "<script>displayLaonLaan();</script>"; // Call the function to display data for Tutuban
            }
        } else {
...
<div class="modal fade" id="confirmResetTrainModal" tabindex="-1" aria-labelledby="deleteModalTutubanLabel" aria-hidden="true">
   ...
   <div class="content ">
      ...
      <div class="row">
            ...
            <li class="nav-item">
               <a class="nav-link active" data-bs-toggle="tab" href="#LaonLaantoTutuban" style="color:#FFC30B;font-weight:900;">Laon-Laan to Tutuban</a>
            </li>
            <li class="nav-item">
               <a class="nav-link" data-bs-toggle="tab" href="#TutubantoLaonLaan" style="color: #FFC30B;font-weight:900;">Tutuban to Laon-Laan</a>
            </li>
         </ul>
         ...
         <div class="tab-content">
            <div id="LaonLaantoTutuban" class="tab-pane fade show active">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainLaonLaan" name="updateTrainLaonLaan" required onchange="updateTrainLaonLaan()">
            ...
            <table id="laonlaan" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
         <div class="modal-dialog">
            <div class="modal-content">
               ...
               <div class="modal-body">
                  <form id="editForm" action="edit_schedule_laonlaan.php" method="post">
            ...
            <div id="TutubantoLaonLaan" class="tab-pane fade">
            <div class="train" style="padding-top:20px;display:block;padding-left:30px;">
            <select style="width: 243px; display: inline-block; margin-left: auto; margin-right: 11px; margin-top: 0" class="form-select" id="updateTrainTutubanLaonLaan" name="updateTrainTutubanLaonLaan" required onchange="updateTrainTutubanLaonLaan()">
            <option value="" disabled selected>Select Train</option>
                <?php
                foreach ($trains as $train) {
                    echo '<option value="' . $train['train_value'] . '" data-station-id="' . $train['train_no'] . '">' . $train['train_no'] . '</option>';
               ...
               <table id="tutuban_laonlaan" class="table table-striped bord" style="width:95%;margin-left:auto;margin-right:auto;">
        <thead>
            <tr>
            <th style="background-color:#174793;color:#FFF;">Select</th>
            <th style="background-color:#174793;color:#FFF;">No.</th>
            <th style="background-color:#174793;color:#FFF;">Time</th>
            <th style="background-color:#174793;color:#FFF;">Status</th>
            <th style="background-color:#174793;color:#FFF;">Train</th>
            <th style="background-color:#174793;color:#FFF;">Direction</th>
            <th style="background-color:#174793;color:#FFF;">Action</th>
            </tr>
        </thead>
        <tbody id="dynamicTableBodyTutuban">
        <?php displayTutubanLaonLaan();?>
...
<div class="modal fade" id="editModalTutuban" tabindex="-1" aria-labelledby="editModalTutubanLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editFormTutuban" action="edit_schedule_tutuban_laonlaan.php" method="post">
...
function downloadTables() {
    ...
    const table1 = $('#laonlaan').DataTable().rows().data().toArray();
    const table2 = $('#tutuban_laonlaan').DataTable().rows().data().toArray();
    ...
    filteredData1.unshift(['Laon-Laan to Tutuban', `Date: ${formattedDate}`]);
    ...
    filteredData2.unshift(['Tutuban to Laon-Laan', `Date: ${formattedDate}`]);
    ...
    link.setAttribute("download", `schedule_laon-laan_tutuban_${formattedDate}.csv`);
...
function editTutuban(scheduleId) {
    ...
    function confirmDeleteTutuban() {
        ...
        // Add logic for deleting a schedule
        $.ajax({
            url: 'delete_tutuban_laonlaan.php',
            method: 'POST',
            data: { scheduleId: scheduleId },
            success: function(response) {
                // Log the response from the server
...
<script>
    function updateTrainLaonLaan() {
        var selectedTrain = document.getElementById("updateTrainLaonLaan").value;
...
<script>
    function updateTrainTutubanLaonLaan() {
        var selectedTrain = document.getElementById("updateTrainTutubanLaonLaan").value;
        ...
        if (selectedTrain !== "" && selectedSchedules.length > 0) {
            $.ajax({
                type: 'POST',
                url: 'update_train_tutuban_laonlaan.php',
                data: { train: selectedTrain, selectedSchedules: selectedSchedules },
                success: function (response) {
                    location.reload();
                },
