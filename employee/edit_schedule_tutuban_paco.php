...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE tutuban_paco SET time = '$time', status = '$status', direction = '$direction' WHERE schedule_id = $scheduleId";

    if (mysqli_query($conn, $query)) {
        // Show a success message using JavaScript alert
        echo "<script>alert('Schedule updated successfully');window.location = 'schedule_paco.php';</script>";
    } else {
        // Show an error message using JavaScript alert
        echo "<script>alert('Error updating schedule');window.location = 'schedule_paco.php';</script>";
    }
